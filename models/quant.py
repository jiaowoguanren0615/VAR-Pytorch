...
class VectorQuantizer2(nn.Module):
    ...
    def forward(self, f_BChw: torch.Tensor, ret_usages=False) -> Tuple[torch.Tensor, List[float], torch.Tensor]:
        ...
        with torch.cuda.amp.autocast(enabled=False):
            ...
            for si, pn in enumerate(self.v_patch_nums):  # from small to large
                ...
                if self.training:
                    if utils.initialized(): handler = tdist.all_reduce(hit_V, async_op=True)
                ...
                if self.training and utils.initialized():
                    handler.wait()
    ...
    def f_to_idxBl_or_fhat(self, f_BChw: torch.Tensor, to_fhat: bool,
        ...
        torch.Tensor]:  # z_BChw is the feature from inp_img_no_grad
        B, C, H, W = f_BChw.shape
        f_no_grad = f_BChw.detach()
        f_rest = f_no_grad.clone()
        f_hat = torch.zeros_like(f_rest)
