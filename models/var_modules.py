...
class SelfAttention(nn.Module):
    ...
    def forward(self, x, attn_bias):
        ...
        if using_flash or self.using_xform:
            q, k, v = qkv.unbind(dim=2); dim_cat = 1  # q or k or v: BLHc
        else:
        ...
        if using_flash:
            ...
            oup = memory_efficient_attention(q, k, v, attn_bias=None if attn_bias is None else attn_bias.to(
                dtype=q.dtype).expand(B, self.num_heads, -1, -1), p=dropout_p, scale=self.scale).view(B, L, C)
        else:
...
class SABlock(nn.Module):
    def __init__(
        ...
        self.attn = SelfAttention(block_idx=block_idx, embed_dim=embed_dim, num_heads=num_heads, attn_drop=attn_drop,
                                  proj_drop=drop, tau=tau, cos_attn=cos_attn, flash_if_available=flash_if_available)
        ...
        if layer_scale >= 0:
            self.gamma1 = 1 if (self.fused_add_norm_fn is not None and block_idx == 0) else nn.Parameter(
                layer_scale * torch.ones(embed_dim), requires_grad=True)
    ...
    def forward(self, x, cond_BD, attn_bias):
        ...
        main_type = x.dtype
        x = x.float() + self.drop_path(
            self.gamma1 * self.attn(self.norm1(x), attn_bias=attn_bias))  # following flash-attn: using fp32 in residual
        x = x + self.drop_path(
            self.gamma2 * self.ffn(self.norm2(x.to(dtype=main_type))))  # following flash-attn: using fp32 in residual
    ...
    def fused_forward_wo_cond(self, x_and_residual, attn_bias):
        ...
        rowscale1 = drop_path(x=x.new_ones(x.shape[:-1]), drop_prob=self.last_drop_p,
                              training=True) if self.last_drop_p > 0 and self.training else None
        x, residual = self.fused_add_norm_fn(
            ...
            epsilon=self.norm1.eps, rowscale=rowscale1,
            layerscale=self.gamma1 if isinstance(self.gamma1, torch.nn.Parameter) else None, prenorm=True,
            residual_in_fp32=True,
        )
        x = self.attn(x, attn_bias=attn_bias)
        rowscale2 = drop_path(x=x.new_ones(x.shape[:-1]), drop_prob=self.drop_prob,
                              training=True) if self.drop_prob > 0 and self.training else None
        x, residual = self.fused_add_norm_fn(
            x0=x, residual=residual, weight=self.norm2.weight, bias=self.norm2.bias, dropout_p=0.0,  # todo: no drop
            epsilon=self.norm2.eps, rowscale=rowscale2,
            layerscale=self.gamma2 if isinstance(self.gamma2, torch.nn.Parameter) else None, prenorm=True,
            residual_in_fp32=True,
        )
