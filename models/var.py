...
from models.var_modules import AdaLNSABlock, SABlock
...
class VAR(nn.Module):
    def __init__(
        ...
        self.rng = torch.Generator(device=utils.get_device())
        ...
        self.selecting_idx = torch.full((1, num_classes), fill_value=1 / num_classes, dtype=torch.float32,
        ...
        # 4. backbone blocks
        self.shared_ada_lin = nn.Sequential(nn.SiLU(inplace=False), SharedAdaLin(self.D,
                                                                                 6 * self.C)) if shared_aln and self.using_aln else nn.Identity()
        ...
        if self.blocks[-1].fused_add_norm_fn is not None:
            self.gamma2_last = nn.Parameter(self.layer_scale * torch.ones(embed_dim),
                                            requires_grad=True) if self.layer_scale >= 0 else 1
        else:
...
class VARHF(VAR, PyTorchModelHubMixin,
    ...
    def __init__(
        ...
        super().__init__(
            vae_local, num_classes, norm_eps, aln, aln_gamma_init, shared_aln, cond_drop_rate, depth, embed_dim,
            num_heads, mlp_ratio, drop_rate, attn_drop_rate, drop_path_rate, layer_scale, tau, cos_attn, patch_nums,
