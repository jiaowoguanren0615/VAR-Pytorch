...
class VQVAE(nn.Module):
    ...
    def forward(self, inp, ret_usages=False):  # -> rec_B3HW, idx_N, loss
        ...
        h_BChw, usages, vq_loss, mean_entropy_loss = self.quantize(self.quant_conv(self.encoder(inp)),
    ...
    def load_state_dict(self, state_dict: Dict[str, Any], strict=True, assign=False):
        if state_dict['quantize.ema_vocab_hit_SV'].shape[0] != self.quantize.ema_vocab_hit_SV.shape[
            0]:  # load a pretrained VAE, but using a new num of scales
